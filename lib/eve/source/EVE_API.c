...
//static uint32_t RAMCommandBuffer = RAM_CMD;
...
void EVE_Init(void)
{
    ...
    HAL_MemWrite16(REG_HSIZE,   EVE_DISP_WIDTH);
    // Total number of clocks per line
    HAL_MemWrite16(REG_HCYCLE,  EVE_DISP_HCYCLE);
    // Start of active line
    HAL_MemWrite16(REG_HOFFSET, EVE_DISP_HOFFSET);
    // Start of horizontal sync pulse
    HAL_MemWrite16(REG_HSYNC0,  EVE_DISP_HSYNC0);
    // End of horizontal sync pulse
    HAL_MemWrite16(REG_HSYNC1,  EVE_DISP_HSYNC1);
    // Active height of LCD display
    HAL_MemWrite16(REG_VSIZE,   EVE_DISP_HEIGHT);
    // Total number of lines per screen
    HAL_MemWrite16(REG_VCYCLE,  EVE_DISP_VCYCLE);
    // Start of active screen
    HAL_MemWrite16(REG_VOFFSET, EVE_DISP_VOFFSET);
    // Start of vertical sync pulse
    HAL_MemWrite16(REG_VSYNC0,  EVE_DISP_VSYNC0);
    // End of vertical sync pulse
    HAL_MemWrite16(REG_VSYNC1,  EVE_DISP_VSYNC1);
    // Define RGB output pins
    HAL_MemWrite8(REG_SWIZZLE,  EVE_DISP_SWIZZLE);
    // Define active edge of PCLK
    HAL_MemWrite8(REG_PCLK_POL, EVE_DISP_PCLKPOL);

    // Read the  GPIO register for a read/modify/write operation
    regGpio = HAL_MemRead8(REG_GPIO);
    // set bit 7 of  GPIO register (DISP) - others are inputs
    regGpio = regGpio | 0x80;
    // Enable the DISP signal to the LCD panel
    HAL_MemWrite8(REG_GPIO, regGpio);
    ...
    HAL_MemWrite8(REG_PCLK, EVE_DISP_PCLK);
    HAL_MemWrite8(REG_PWM_DUTY, 127);

    // ---------------------- Touch and Audio settings -------------------------

    // Eliminate any false touches
    HAL_MemWrite16(REG_TOUCH_RZTHRESH, 1200);

    // turn recorded audio volume down
    HAL_MemWrite8(REG_VOL_PB, ZERO);
    // turn synthesizer volume down
    HAL_MemWrite8(REG_VOL_SOUND, ZERO);
    // set synthesizer to mute
    HAL_MemWrite16(REG_SOUND, 0x6000);
...
void EVE_LIB_BeginCoProList(void)
{
    ...
    HAL_SetWriteAddress(RAM_CMD + HAL_GetCmdPointer());
}
...
void EVE_LIB_GetProps(uint32_t *addr, uint32_t *width, uint32_t *height)
{
	...
	// Obtain the results from the RAM_CMD in the CoProcessor.
	*addr = HAL_MemRead32(RAM_CMD + ((WritePointer + (2 * sizeof(uint32_t))) & (FT_CMD_FIFO_SIZE - 1)));
	*width = HAL_MemRead32(RAM_CMD + ((WritePointer + (3 * sizeof(uint32_t))) & (FT_CMD_FIFO_SIZE - 1)));
	*height = HAL_MemRead32(RAM_CMD + ((WritePointer + (4 * sizeof(uint32_t))) & (FT_CMD_FIFO_SIZE - 1)));
}
...
{
    HAL_Write32(CLEAR_COLOR_RGB(R, G, B));
    HAL_IncCmdPointer(4);
}

void EVE_CLEAR_COLOR(uint32_t c)
{
    HAL_Write32(CLEAR_COLOR(c));
    HAL_IncCmdPointer(4);
}

void EVE_CLEAR(uint8_t C, uint8_t S, uint8_t T)
{
    HAL_Write32(CLEAR((C & 0x01),(S & 0x01),(T & 0x01)));
    HAL_IncCmdPointer(4);
}

void EVE_COLOR_RGB(uint8_t R, uint8_t G, uint8_t B)
{
    HAL_Write32(COLOR_RGB(R, G, B));
    HAL_IncCmdPointer(4);
}
...
{
    HAL_Write32(SAVE_CONTEXT());
    HAL_IncCmdPointer(4);
}

void EVE_RESTORE_CONTEXT(void)
{
    HAL_Write32(RESTORE_CONTEXT());
    HAL_IncCmdPointer(4);
}
...
{       
    HAL_Write32(CMD_SWAP);
    HAL_IncCmdPointer(4);
}
  
void EVE_CMD_INFLATE(uint32_t ptr)
{
    HAL_Write32(CMD_INFLATE);
    HAL_Write32(ptr);
    HAL_IncCmdPointer(8);
}
...
{
    HAL_Write32(BITMAP_TRANSFORM_A(a)); //    ((21UL << 24) | (((a)&131071UL)<<0))
    HAL_IncCmdPointer(4);
}

void EVE_BITMAP_TRANSFORM_B(long b)
{
    HAL_Write32(BITMAP_TRANSFORM_B(b)); //  ((22UL << 24) | (((b)&131071UL)<<0))
    HAL_IncCmdPointer(4);
}

void EVE_BITMAP_TRANSFORM_C(long c)
{
    HAL_Write32(BITMAP_TRANSFORM_C(c)); //  ((23UL << 24) | (((c)&16777215UL)<<0))
    HAL_IncCmdPointer(4);
}

void EVE_BITMAP_TRANSFORM_D(long d)
{
    HAL_Write32(BITMAP_TRANSFORM_D(d)); //   ((24UL << 24) | (((d)&131071UL)<<0))
    HAL_IncCmdPointer(4);
}

void EVE_BITMAP_TRANSFORM_E(long e)
{
    HAL_Write32(BITMAP_TRANSFORM_E(e)); //   ((25UL << 24) | (((e)&131071UL)<<0))
    HAL_IncCmdPointer(4);
}

void EVE_BITMAP_TRANSFORM_F(long f)
{
    HAL_Write32(BITMAP_TRANSFORM_F(f)); //  ((26UL << 24) | (((f)&16777215UL)<<0))
    HAL_IncCmdPointer(4);
}

void EVE_CMD_INTERRUPT(uint32_t ms)
{       
    HAL_Write32(CMD_INTERRUPT);
    HAL_Write32(ms);
    HAL_IncCmdPointer(8);
}
...
{       
    HAL_Write32(CMD_BITMAP_TRANSFORM);
    HAL_Write32(x0);
    HAL_Write32(y0);
    HAL_Write32(x1);
    HAL_Write32(y1);
...
{          
    HAL_Write32(CMD_DLSTART);
    HAL_IncCmdPointer(4);
}

void EVE_CMD_SNAPSHOT(uint32_t ptr)
{            
    HAL_Write32(CMD_SNAPSHOT);
    HAL_Write32(ptr);
    HAL_IncCmdPointer(8);
}
...
{        
    HAL_Write32(CMD_INT_SWLOADIMAGE);
    HAL_Write32(ptr);
    HAL_Write32(options);
    HAL_IncCmdPointer(12);
}
...
{
    HAL_Write32(CMD_VIDEOSTART);
    HAL_IncCmdPointer(4);
}

void EVE_CMD_SETBASE(uint32_t base)
{
    HAL_Write32(CMD_SETBASE);
    HAL_Write32(base);
    HAL_IncCmdPointer(8);
}
