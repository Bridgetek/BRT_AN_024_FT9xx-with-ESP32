...
static BlockLink_t xStart[MAX_HEAP_TYPES], xEnd[MAX_HEAP_TYPES];
...
size_t xBlockSize;																	\
	...
	for( pxIterator = &xStart[CurHeapIdx]; pxIterator->pxNextFreeBlock->xBlockSize < xBlockSize; pxIterator = pxIterator->pxNextFreeBlock )	\
	{																				\
...
void *pvReturn = NULL;
	...
	kernel_do_on_malloc_entry();
	{
		...
		if( xHeapHasBeenInitialised[CurHeapIdx] == pdFALSE )
		{
			prvHeapInit();
			xHeapHasBeenInitialised[CurHeapIdx] = pdTRUE;
		}
		...
		if( ( xWantedSize > 0 ) && ( xWantedSize < MaxHeapSize[CurHeapIdx] ) )
		{
...
uint8_t *pucAlignedHeap;
	...
	MaxHeapSize[Kernel_Heap] = configADJUSTED_HEAP_SIZE;
	...
	xEnd[CurHeapIdx].xBlockSize = MaxHeapSize[CurHeapIdx];
	xEnd[CurHeapIdx].pxNextFreeBlock = NULL;

	/* To start with there is a single free block that is sized to take up the
	entire heap space. */
	pxFirstFreeBlock = ( void * ) pucAlignedHeap;
	pxFirstFreeBlock->xBlockSize = MaxHeapSize[CurHeapIdx];
	pxFirstFreeBlock->pxNextFreeBlock = &xEnd[CurHeapIdx];
}
/*-----------------------------------------------------------*/
...
bool xPortSwitchHeap(e_HeapTypes heap, uint32_t heapStartAdr){
	if(heap == User_Heap){
		...
		// Initialise the user heap
			MaxHeapSize[User_Heap] = (FT32_SRAM_END_ADDRESS - heapStartAdr) & ~(uint32_t)0x03; // align to word
			prvHeapInit();
		}
